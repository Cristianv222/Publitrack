...
def check_audio_storage_settings(app_configs, **kwargs):
    ...
    try:
        ...
        errors.append(
            Warning(
                f'Error en verificaciones de almacenamiento: {str(e)}',
                hint='Verifica la configuración del módulo',
...
def check_file_upload_settings(app_configs, **kwargs):
    ...
    try:
        ...
        errors.append(
            Warning(
                f'Error verificando configuración de subida de archivos: {str(e)}',
                hint='Revisa la configuración en settings.py',
...
def check_content_management_data(app_configs, **kwargs):
    """
    Verifica la integridad de datos del módulo - VERSIÓN SEGURA
    """
    errors = []
    
    try:
        # Importación lazy y segura
        ...
        try:
            from apps.content_management.models import CuñaPublicitaria, ArchivoAudio, CategoriaPublicitaria
        except ImportError:
        ...
        try:
            ...
            pass  # Skip si hay problemas con la BD
        ...
        try:
            ...
            for archivo in ArchivoAudio.objects.all()[:100]:  # Limitar para evitar sobrecarga
            ...
            if archivos_huerfanos > 0:
                errors.append(
                    Warning(
                        f'{archivos_huerfanos} archivos de audio sin archivo físico (muestra de 100)',
        ...
        # Solo reportar como warning, no como error crítico
        errors.append(
            Warning(
                f'No se pudieron verificar todos los datos del módulo: {str(e)}',
                hint='Esto puede ser normal durante la inicialización. Ejecuta python manage.py check más tarde.',
...
def check_database_indexes(app_configs, **kwargs):
    """
    Verifica que los índices de base de datos estén optimizados - VERSIÓN SEGURA
    """
    errors = []
                ...
                try:
                    ...
                    with connection.cursor() as cursor:
                        cursor.execute("""
                            SELECT COUNT(*) 
                            FROM pg_indexes 
                            WHERE tablename LIKE '%content_management%'
                        """)
                        indices_count = cursor.fetchone()[0]
                        
                        if indices_count < 5:  # Debería haber al menos 5 índices
                            errors.append(
                                Warning(
                    ...
                    pass  # Skip si no se puede acceder a metadatos
        
        except ImportError:
            pass  # Skip si no se pueden importar modelos
        
    except Exception as e:
        errors.append(
            Info(
                f'No se pudo verificar índices de base de datos: {str(e)}',
                hint='Esto es normal si no tienes acceso completo a metadatos de la BD',
...
def check_module_integrations(app_configs, **kwargs):
    ...
    try:
        ...
        errors.append(
            Warning(
                f'Error verificando integraciones: {str(e)}',
                hint='Algunos módulos pueden no estar disponibles',
                id='content_management.W015',
            )
        )
...
def check_user_groups_and_permissions(app_configs, **kwargs):
    """
    Verifica que los grupos de usuarios y permisos estén configurados - VERSIÓN SEGURA
    """
    errors = []
    ...
    try:
        ...
        try:
            ...
            content_type = ContentType.objects.get_for_model(CuñaPublicitaria)
            ...
            pass  # Skip si no se pueden verificar permisos
...
def run_all_checks():
    ...
    try:
        ...
        # Filtrar solo los issues de este módulo
        module_issues = [issue for issue in issues if 'content_management' in str(issue.id)]
        
        # Categorizar issues
        errors = [issue for issue in module_issues if issue.level >= 40]  # ERROR
        warnings = [issue for issue in module_issues if 30 <= issue.level < 40]  # WARNING
        infos = [issue for issue in module_issues if issue.level < 30]  # INFO
        ...
        return {
            ...
            'infos': 0,
            'details': {
                'errors': [f'Error ejecutando verificaciones: {str(e)}'],
                'warnings': [],
                'infos': [],
            }
        }