...
class ContratoGenerado(models.Model):
    ...
    spots_dia = models.IntegerField(default=1, verbose_name="Spots por Día")
    duracion_spot = models.IntegerField(default=30, verbose_name="Duración del Spot (segundos)")
    
    # Archivo generado - SE GUARDARÁ EN /app/media/contratos_generados/
    archivo_generado = models.FileField(
        upload_to='contratos_generados/',  # ← Aquí se guardan los archivos
    ...
    class Meta:
        ...
        verbose_name_plural = "Contratos Generados"
        ordering = ['-created_at']
        db_table = 'content_contrato_generado'
    
    def __str__(self):
        cliente_nombre = self.cliente.empresa or self.cliente.get_full_name()
        return f"{self.numero_contrato} - {cliente_nombre}"
